...
from config.config import (
    DEFAULT_DOWNLOAD_PATH, MAX_DOWNLOAD_THREADS, TARGET_SUB_PATH, RESOURCE_ID
)
...
class DownloadProcessor:
    ...
    def __init__(self, custom_download_path: str = None, config_overrides: dict = None):
        ...
        self.finished_log_file = os.path.join(self.download_base_path, finished_log_filename)
        
        # Ensure download directory exists before initializing components
        os.makedirs(self.download_base_path, exist_ok=True)
        
        # Initialize components
        self.catalog_manager = CatalogManager(self.catalog_file, resource_id=self.resource_id)
        ...
        print(f"  - Target Sub Path: '{self.target_sub_path}' (empty means process all)")
        ...
        while True:
            ...
            if self.downloader.download_file(fullpath, filename, local_path):
                ...
                with self.lock_download_count:
                    ...
                    current_total_downloaded_by_all_threads = self.downloaded_files_count

                if not self.downloader.record_download_info(fullpath, filename, size):
                    print(f"Thread {current_thread_id}: Failed to record download information for: {filename}")
                
                if current_total_downloaded_by_all_threads % self.sleep_after_files == 0 and current_total_downloaded_by_all_threads > 0:
                    print(f"Thread {current_thread_id}: Pausing for {self.sleep_interval} second(s) after {current_total_downloaded_by_all_threads} total downloads by the process...")
    ...
    def process_catalog_and_download(self) -> None:
        ...
        if self.target_sub_path and self.target_sub_path.strip():
            normalized_target_sub_path = self.target_sub_path.strip().replace("\\", "/") # Normalize slashes
            print(f"Filtering catalog paths by TARGET_SUB_PATH: '{normalized_target_sub_path}'")
            for p in all_catalog_paths:
                normalized_catalog_path = p.replace("\\", "/") # Normalize slashes for comparison
                if normalized_catalog_path.startswith(normalized_target_sub_path):
                    paths_to_process.append(p)
            if not paths_to_process:
                print(f"No paths in the catalog match the TARGET_SUB_PATH: '{normalized_target_sub_path}'. No files will be downloaded.")
        ...
        print(f"Processing {len(paths_to_process)} paths using up to {self.max_threads} threads...")
        print(f"Table: {table}")
        ...
        for i, path in enumerate(paths_to_process, 1):
            ...
            print(f"Producer: Queued {len(file_list)} files from path: {path}. Total tasks queued so far: {tasks_added_to_queue}")

        print(f"Producer: All {tasks_added_to_queue} download tasks have been added to the queue.")

        # Signal worker threads to stop by sending sentinel values
        print(f"Producer: Sending {self.max_threads} sentinel values to stop worker threads...")
        for _ in range(self.max_threads):
        ...
        print("Producer: Waiting for all tasks in queue to be processed (including sentinels)...")
        ...
        print(f"Total files successfully downloaded: {self.downloaded_files_count}") # This count is from workers
    ...
    def run_full_process(self) -> None:
        ...
        print(f"- Downloaded files: Organized in subdirectories under {self.download_base_path}") 